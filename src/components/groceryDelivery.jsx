...
import groceryCities from "../utils/groceryDelivery"
...
export default function FoodDelivery() {
    ...
    let len = 0;
    if(!open) {
        for(const val of groceryCities.categories) {
            len++;
            if(len < 20) {
                citiesList.push(val);
            }
        ...
        citiesList = groceryCities.categories;
    }
    return (
        <div className="mt-[80px] ml-10 max-w-[1000px]">
            <h2 className="font-bold text-[24px] mb-3">Cities with grocery delivery</h2>
